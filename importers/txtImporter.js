...
function normalizarLinhaTransferencia(linha) {
...
function extrairTransferencia(descricaoBruta) {
    ...
    const rota = transferenciaRaw.match(
        /N:(L\d+|LT|TOMATE)→B\/C:(L\d+|LT|TOMATE)/i
    );
    ...
    return {
        ...
        transferencia: {
            ...
            linhaOrigemNegra: rota
                ? normalizarLinhaTransferencia(rota[1])
                : null,

            linhaDestinoBrancaCinza: rota
                ? normalizarLinhaTransferencia(rota[2])
                : null
